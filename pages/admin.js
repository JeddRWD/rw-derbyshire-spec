...
export default function AdminPage() {
  ...
  const [duplicateFromSiteId, setDuplicateFromSiteId] = useState("");
  const [duplicateToSiteId, setDuplicateToSiteId] = useState("");
  ...
  const duplicateSiteSpecs = async (e) => {
    ...
    if (!duplicateFromSiteId || !duplicateToSiteId) {
      return alert("Please select both source and target sites.");
    }

    if (String(duplicateFromSiteId) === String(duplicateToSiteId)) {
      return alert("Source and target site cannot be the same.");
    }

    const specsToCopy = specs.filter(
      (spec) => String(spec.site_id) === String(duplicateFromSiteId)
    );

    if (specsToCopy.length === 0) {
      return alert("No specs found on the source site.");
    }

    const rowsToInsert = specsToCopy.map((spec) => ({
      title: spec.title,
      body: spec.body,
      site_id: duplicateToSiteId,
    ...
    setDuplicateFromSiteId("");
    setDuplicateToSiteId("");
    loadData();
    alert("All specs copied successfully to the selected site");
  };
    ...
    <>
      <div style={{ minHeight: "100vh", background: "#f4f6f8", padding: 30 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <form onSubmit={copySpecToSites} style={{ ...cardStyle, marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button
                  type="button"
                  onClick={() =>
                    setCopySiteIds(availableCopySites.map((site) => site.id))
                  }
          ...
          <form
            onSubmit={duplicateSiteSpecs}
            style={{ ...cardStyle, marginBottom: 20 }}
          >
            <h2>Duplicate All Specs From One Site To Another</h2>

            <select
              value={duplicateFromSiteId}
              onChange={(e) => setDuplicateFromSiteId(e.target.value)}
              style={inputStyle}
            >
            ...
            <select
              value={duplicateToSiteId}
              onChange={(e) => setDuplicateToSiteId(e.target.value)}
              style={inputStyle}
            >
